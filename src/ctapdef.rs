...
pub(crate) fn get_ctap_status_message(status: u8) -> String {
    match status {
        ...
        0x06 => "0x06 CTAP1_ERR_CHANNEL_BUSY Channel busy. Client SHOULD retry the request after a short delay. Note that the client may abort the transaction if the command is no longer relevant.".to_string(),
        ...
        0x29 => "0x29 CTAP2_ERR_NOT_BUSY Authenticator cannot cancel as it is not busy.".to_string(), // Not in current spec
        0x2A => "0x2A CTAP2_ERR_NO_OPERATION_PENDING No outstanding operations.".to_string(), // Not in current spec
        ...
        0x2F => "0x2F CTAP2_ERR_USER_ACTION_TIMEOUT A user action timeout occurred.".to_string(),
        ...
        0x33 => "0x33 CTAP2_ERR_PIN_AUTH_INVALID PIN authentication, pinUvAuthParam, verification failed.".to_string(),
        0x34 => "0x34 CTAP2_ERR_PIN_AUTH_BLOCKED PIN authentication using pinUvAuthToken blocked. Requires power cycle to reset.".to_string(),
        0x35 => "0x35 CTAP2_ERR_PIN_NOT_SET No PIN has been set.".to_string(),
        0x36 => "0x36 CTAP2_ERR_PIN_REQUIRED A pinUvAuthToken is required for the selected operation.".to_string(),
        0x37 => "0x37 CTAP2_ERR_PIN_POLICY_VIOLATION PIN policy violation. Currently only enforces minimum length.".to_string(),
        0x38 => "0x38 CTAP2_ERR_PIN_TOKEN_EXPIRED pinToken expired on authenticator.".to_string(), // "Reserved for Future Use" in current spec
        ...
        0xE1..=0xEE => "0xE0 CTAP2_ERR_EXTENSION Extension specific error.".to_string(),
